...
const nextHeartSpot = (hearts) => {

  const heartSpots = GAME_WIDTH / HEART_WIDTH;

  const spotsOccupied = [false, false, false, false, false, false, false, false];

  hearts.forEach((heart) => {
    spotsOccupied[heart.spot] = true;
  });

  let potentialSpot = undefined;
  while (potentialSpot === undefined || spotsOccupied[potentialSpot]) {

    potentialSpot = Math.floor(Math.random() * heartSpots);
